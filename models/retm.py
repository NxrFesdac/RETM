...
class RETM(object):
    ...
    def _train(self, epoch):
        ...
        for idx, ind in enumerate(indices):
            ...
            recon_loss, kld_theta, recon_loss_pplxity = self.model(
            ...
            self.optimizer.step()
            ##Perplexity on training
            pplxity_loss = recon_loss_pplxity / sums.squeeze()
            pplxity_loss = pplxity_loss.mean().item()
            pplxity_acc_loss += pplxity_loss
        ...
        ppl_dc = round(math.exp(pplxity_acc_loss / cnt), 1)
        self.train_perplexity = ppl_dc
        cur_loss = round(acc_loss / cnt, 2)
        cur_kl_theta = round(acc_kl_theta_loss / cnt, 2)
        cur_real_loss = round(cur_loss + cur_kl_theta, 2)

        if self.debug_mode:
            print('Epoch {} - Learning Rate: {} - KL theta: {} - Rec loss: {} - NELBO: {} - Perplexity: {}'.format(
                epoch, self.optimizer.param_groups[0]['lr'], cur_kl_theta, cur_loss, cur_real_loss, ppl_dc))
    ...
    def get_topics(self, top_n_words=10) -> List[str]:
        ...
        with torch.no_grad():
            ...
            for k in range(self.num_topics):
                ...
                topic_values.append(sorted(gamma.cpu().numpy(), reverse=True)[:top_n_words])

            return topics, topic_values
